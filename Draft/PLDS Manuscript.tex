...
\hline
\textbf{E Step}\\
0. Initialize $\Theta =\{A,C,R,\mathbf{\pi}_0\}$ with SVD, if first loop\\
...
$R$ matrix is initialized as the identify matrix, while $\mathbf{\pi}_0$ as $\mathbf{0}$ vector. For $A$ and $C$, first denote $\mathbf{Y} = \left[\mathbf{y_1},\cdots,\mathbf{y_T}\right]$, a $p\times T$ matrix. Then the singular value decomposition (SVD) of $\mathbf{Y}$ is
...
\captionof{figure}{Sample time series plot. The dotted green curve stands for the $60\%$ confidence band given by PLDS model. The time series is the average of signals from a subsample. A subsample of 20 voxels are picked to avoid big covariance matrices calculation. The left vertical axis is used for the true signal and PLDS predictions, while the one on the right is for PCA predictions.}
\label{fig:samplets}
\end{center}
