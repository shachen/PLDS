...
\title{Reduced Rank M-Estimation for High-Dimensional Linear Dynamical System Identification
...
%Let $\mathbf{z} = (x_{11},x_{21},\ldots,x_{T1},x_{12},x_{22},\ldots,x_{T2},\ldots,x_{1d},x_{2d},\ldots,x_{Td})^{\prime}$ be a vector from rearranging $\bX$. In addition, denote
%\[
...
%\]
%where $Z=\biggl(\bx_0,\ldots,\bx_{T-1}\biggr)$.
...
%\]
%
%where $\bX=\left(\mathbf{x_1}\cdots\mathbf{x_N}\right)$.
...
%\end{equation}
%where \textbf{\underline{z}} is a vector from concatenating $\mathbf{x_1},\cdots, \mathbf{x_T}$ and \textbf{\underline{W}} is a block diagonal matrix with diagonal component $\bX'^T\text{, where }\bX'=\left(\mathbf{x_0},\cdots,\mathbf{x_{T-1}}\right)$.
%
...
%\[
%\|\textbf{\underline{X}}\by\|^2=\sum\limits_{i=1}^V \|\bX^T \by_i\|^2 \leq \sum\limits_{i=1}^V \text{L}^2 \|\by_i\|^2 = \text{L}^2
%\]
%where $\by_i$ is the $i\text{-th}$ length-Q subarray of $\by$ and $\|\by\| = 1$.
...
%Specifically, the following simulation is performed: the proposed \mrsid~model is used to make 1-step, 2-step, $\cdots$, k-step predictions. Similar predictions are made with a PCA-based procedure and the prediction accuracies are compared.

%In the PCA-based procedure, denote $\bY = \left[\mathbf{y_1},\cdots,\mathbf{y_T}\right]$, a $p\times T$ matrix. The singular value decomposition (SVD) of $\bY$ is
%\begin{equation*}
%    \bY = \mathbf{UDV^{\prime}} = \mathbf{U}_{p\times T}\bX_{T \times T}
%\end{equation*}
%Based on this decomposition, $\mathbf{U}_{p\times T}$ is used as the estimation of $C$ matrix, and the columns of $\bX_{T \times T}$ are used in a vector autoregressive (VAR) model to estimate the coefficient matrix $A$. With an estimation for $C$ and for $A$, one can begin by computing a 1-step prediction for $\bx_{T+1}$, then for $\by_{T+1}$. With these predictions, one could further generate the 2-step, 3-step, ., k-step predictions.



\begin{center}
\includegraphics[scale=0.7]{./figures/est-pred-accuracy.pdf}
\captionof{figure}{Estimation and prediction accuracies. The x-axis is the penalty size under log scale, while y-axis is the estimation and prediction accuracies. One can see that the penalty that yields the most accurate estimation also gives the best predictions.}
...
\begin{itemize}
 %\item \mrsid~prediction accuracy is almost the same as the PCA-based procedure when there is no penalty. This is consistent with the fact that the generic LDS is equivalent to PCA with some constraints on parameters.
...
% The biological meaning of those regions needs to be further validated. 
It is helpful to compare those regions to other existing parcellations of the mortor cortex. As an example, the blue region in Figure \oldref{fig:3d} accurately matches the dorselmedical (DM) parcel of the five-region parcellation proposed by Nebel MB et al. \cite{nebel2014disruption}.
\begin{center}
\[
\begin{array}{lll}
