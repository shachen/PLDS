...
Without applying further constraints, the model itself is unidentifiable and too general to be useful. Supplemental constraints are are thus introduced to address both identifiability and utility. Three basic constraints are required to make the model identifiable:
\vspace*{-3mm}
...
\begin{aligned}
	...
	&\mathbf{y_t}=C\mathbf{x}_t+\mathbf{v}_t,\qquad \mathbf{v}_t\sim N(\mathbf{0},R),\quad C \text{ has smooth columns}
\end{aligned}
\end{equation}
...
Specifically, one way to find the maximum likelihood solution is through iterative techniques such as expectation maximization (EM) \cite{shumway1982approach}. The detailed EM steps for a generic LDS can be found in Zoubin and Geoffrey (1996) \cite{ghahramani1996parameter}. An alternative approach is to use subspace identification methods such as N4SID and PCA-ID to compute an asymptotically unbiased solution in closed form \cite{van1994n4sid} \cite{doretto2003dynamic}. In practice, determining an initial solution with subspace identification and then refining the solution with EM is a worthwhile approach \cite{bootslearning}.

However, the above solutions can not be directly applied to model \ref{eqn:penaltylik} due to the introduced penalty terms. A new algorithm is then developed and detailed as follows.

By the chain rule, the likelihood in model \ref{eq:model0} is
\begin{equation*}\label{eqn:likelihood}
P(\{\mathbf{x}\},\{\mathbf{y}\})=P(\mathbf{x_0})\prod\limits_{t=1}^{T}P(\mathbf{x_t}|\mathbf{x_{t-1}})\prod\limits_{t=1}^{T} P(\mathbf{y_t}|\mathbf{x_t})=\prod\limits_{t=1}^{T}P(\mathbf{x_t}|\mathbf{x_{t-1}})\prod\limits_{t=1}^{T} P(\mathbf{y_t}|\mathbf{x_t})\mathbbm{1}_{\mathbf{\pi}_0}(\mathbf{x}_0)
\end{equation*}
...
\begin{aligned}
P(\mathbf{y_t}|\mathbf{x_t})&=\text{exp}\left\{-\frac{1}{2}[\mathbf{y_t}-C\mathbf{x_t}]^{\prime}R^{-1}[\mathbf{y_t}-C\mathbf{x_t}]\right\} (2\pi)^{-p/2}|R|^{-1/2}\\
P(\mathbf{x_t}|\mathbf{x_{t-1}})
%%&=\text{exp}\left\{-\frac{1}{2}[\mathbf{x_t}-A\mathbf{x_{t-1}}]^{\prime}Q^{-1}[\mathbf{x_t}-A\mathbf{x_{t-1}}]\right\}(2\pi)^{-d/2}|Q|^{-1/2}\\
&=\text{exp}\left\{-\frac{1}{2}[\mathbf{x_t}-A\mathbf{x_{t-1}}]^{\prime}[\mathbf{x_t}-A\mathbf{x_{t-1}}]\right\}(2\pi)^{-d/2}.
\end{aligned}
\end{equation*}
...
\begin{split}
\text{log } P(\{\mathbf{x}\},\{\mathbf{y}\})=&-\sum\limits_{t=1}^{T}\big(\frac{1}{2}[\mathbf{y_t}-C\mathbf{x_t}]^{\prime}R^{-1}[\mathbf{y_t}-C\mathbf{x_t}]\big)-\frac{T}{2}\text{log}|R|\\
&-\sum\limits_{t=1}^{T}\big(\frac{1}{2}[\mathbf{x_t}-A\mathbf{x_{t-1}}]^{\prime}[\mathbf{x_t}-A\mathbf{x_{t-1}}]\big)-\frac{T}{2}\text{log}|\mathbf{I}|+ \text{log}(\mathbbm{1}_{\mathbf{\pi}_0}(\mathbf{x}_0)).
\end{split}
\end{equation}
...
\begin{split}
\hat{\Theta}=\text{arg}\min_{\substack{\Theta}}\biggl\{&\sum\limits_{t=1}^{T}\big(\frac{1}{2}[\mathbf{y_t}-C\mathbf{x_t}]^{\prime}R^{-1}[\mathbf{y_t}-C\mathbf{x_t}]\big)-\frac{T}{2}\text{log}|R|\\
&+\sum\limits_{t=1}^{T}\big(\frac{1}{2}[\mathbf{x_t}-A\mathbf{x_{t-1}}]^{\prime}[\mathbf{x_t}-A\mathbf{x_{t-1}}]\big)-\frac{T}{2}\text{log}|\mathbf{I}| - \text{log}(\mathbbm{1}_{\mathbf{\pi}_0}(\mathbf{x}_0))\\
...
\]
This quantity depends on three expectations: $E[\mathbf{x}_t|\{\mathbf{y}\}]$, $E[\mathbf{x}_t\mathbf{x}_t^{\prime}|\{\mathbf{y}\}]$ and $E[\mathbf{x}_t\mathbf{x}_{t-1}^{\prime}|\{\mathbf{y}\}]$. We denote them by the symbols:
\begin{equation}\label{eq:expecs}
\begin{aligned}
\hat{\mathbf{x}}_t \quad &\equiv \quad E[\mathbf{x_t}|\{\mathbf{y}\}]\\
P_t \quad &\equiv \quad E[\mathbf{x}_t\mathbf{x}_t^{\prime}|\{\mathbf{y}\}]\\
P_{t,t-1} \quad &\equiv \quad E[\mathbf{x}_t\mathbf{x}_{t-1}^{\prime}|\{\mathbf{y}\}].
\end{aligned}
\end{equation}
...
\begin{aligned}
\frac{\partial \mathbf{\Phi}}{\partial R^{-1}} &= \frac{T}{2}R - \sum\limits_{t=1}^T\bigl(\frac{1}{2}\mathbf{y}_t\mathbf{y}_t^{\prime} - C\hat{\mathbf{x}}_t\mathbf{y}_t^{\prime}+\frac{1}{2}CP_tC^{\prime}\bigr) =0 \\
& R = \frac{1}{T}\sum\limits_{t=1}^{T}(\mathbf{y}_t\mathbf{y}_t^{\prime}-C^{\text{new}}\hat{\mathbf{x}}_t\mathbf{y}_t^{\prime})\\
& R^{\text{new}} =\text{Diag}\biggl\{\frac{1}{T}\sum\limits_{t=1}^{T}(\mathbf{y}_t\mathbf{y}_t^{\prime}-C\hat{\mathbf{x}}_t\mathbf{y}_t^{\prime})\biggr\}
\end{aligned}
\end{equation*}
...
\sum\limits_{t=1}^{T}\left(\frac{1}{2}[\mathbf{y}_t-C\mathbf{x}_t]^{'}R^{-1}[\mathbf{y}_t-C\mathbf{x}_t]\right)+\lambda_2 \|C\|_2 = f_{\lambda_2}(C;\{\mathbf{x}\},\{\mathbf{y}\}).
\end{equation}
...
\begin{equation*}
\sum\limits_{t=1}^{T}\left(\frac{1}{2}[R^{-\frac{1}{2}}y_t-R^{-\frac{1}{2}}Cx_t]^{'}[R^{-\frac{1}{2}y_t}-R^{-\frac{1}{2}}Cx_t]\right) + \lambda_2 \|R^{-\frac{1}{2}}C\|
\end{equation*}
Let
\begin{equation*}
\mathbf{w} = (y_{11},\ldots,y_{T1},y_{12},\ldots,y_{T2},\ldots,y_{1p},\ldots,y_{Tp})^{\prime}
\end{equation*}
...
\mathbf{c}^{\text{old}} = (C_{11}^{\text{old}},\ldots,C_{1d}^{\text{old}},C_{21}^{\text{old}},\ldots,C_{2d}^{\text{old}},C_{p1}^{\text{old}},\ldots,C_{pd}^{\text{old}})^{\prime}
\end{equation}
...
\begin{aligned}
\mathbf{c}^{\text{new}} &= (\mathbf{W}^{\prime}\mathbf{W} + \lambda_2\mathbf{I})^{-1}\mathbf{W}^{\prime}\mathbf{w}\\
...
\sum\limits_{t=1}^{T}\big(\frac{1}{2}[\mathbf{x}_t-A\mathbf{x}_{t-1}]^{\prime}[\mathbf{x}_t-A\mathbf{x}_{t-1}]\big)+\lambda_1 \|A\|_1 = f_{\lambda_1}(A;\{\mathbf{x}\},\{\mathbf{y}\}).
\end{equation}
...
\end{equation}
where $\mathbf{z}$ is a $Td \times 1$ vector from rearranging $\{\mathbf{x}\}$ and $\mathbf{Z}$ is a block diagonal matrix with diagonal component $Z^{\prime} =(\mathbf{x}_0,\ldots,\mathbf{x}_{T-1})^{\prime}$. Unfortunately, equation \ref{eq:penaltylik21} does not have closed form solution due to the $L-1$ term.
...
FISTA requires calculating the Lipschitz constant $L$ for $\nabla\mathbf{g(z)}=\mathbf{Z}^{\prime}(\mathbf{Z}\mathbf{a} -\mathbf{z})$, where $\mathbf{g}(\mathbf{z})=\|\mathbf{Z}^{\prime}\mathbf{a} -\mathbf{z}\|_2^2$. Denote $\|Z\|$ as the induced norm of matrix $Z$, then $L$ is
\[
L = \sup_{\substack{x\neq y}}\frac{\|\mathbf{Z^{\prime}}(\mathbf{Z}x- \mathbf{Z}y)\|}{\|x-y\|}=\sup_{\substack{x\neq 0}}\frac{\|\mathbf{Z^{\prime}Z}x\|}{\|x\|}\leq\|\mathbf{Z}^{\prime}\|\|\mathbf{Z}\| = \|Z^{\prime}\|\|Z\|.
\]
...
A^{\text{new}} = \text{FISTA}(\|\mathbf{Z}^{\prime}\mathbf{a}^{\text{old}} -\mathbf{z}\|_2^2,\quad \lambda_2)
\end{equation}
...
1. $R^{\text{new}}=\text{Diag}\biggl\{\frac{1}{T}\sum\limits_{t=1}^{T}(\mathbf{y}_t\mathbf{y}_t^{\prime}-C^{\text{old}} \hat{\mathbf{x}}_t\mathbf{y}_t^{\prime})\biggr\}$\\
...
\begin{equation}\label{eq:initial}
    \mathbf{Y} = \mathbf{UDV^{\prime}} \approx \mathbf{U}_{p \times d} \mathbf{D}_{d \times d} \mathbf{V}_{d \times T}^{\prime} =\mathbf{U}_{p\times d}\mathbf{X}_{d \times T}
\end{equation}
where $\mathbf{U}_{p \times d}$ is the first $d$ columns of $\mathbf{U}$ and $\mathbf{D}_{d\times d}$ is the upper left block of $\mathbf{D}$. This notation also applies to $\mathbf{V}^{\prime}_{d \times T}$.
...
\[
K_t=V_t^{t-1}C^{\prime}(CV_t^{t-1}C^{\prime}+R)^{-1}
\]
...
\[
(CV_t^{t-1}C^{\prime}+R)^{-1} = R^{-1} - R^{-1}C[(V_t^{t-1})^{-1} + C^{\prime}R^{-1}C]^{-1}C^{\prime}R^{-1}
\]
...
\captionof{figure}{Estimation and prediction accuracies.}
...
\begin{itemize}
 %\item The PLDS prediction accuracy is almost the same as the PCA-based procedure when there is no penalty. This is consistent with the fact that the generic LDS is equivalent to PCA with some constraints on parameters.
...
\end{array}
\]
\captionof{figure}{Eigen-values and Corresponding Profile Likelihood Plot}
\label{fig:eigvals}
\end{center}
...
\captionof{figure}{Similarities among the four estimated $A$ matrices. The distance $d(\cdot,\cdot)$ is used in this figure. As one can see, the two yellow off-diagonal pixels has the minimum distances, which correspond to the pairs of $(A_{11},A_{12})$ and $(A_{21},A_{22})$ respectively.}
\label{fig:matsim} 
\end{center}
...
\end{array}
\]
\captionof{figure}{3D Rendering of Columns of Matrix $C$}
\label{fig:3d}
\end{center}
...
\captionof{figure}{Sample time series plot. The dotted green curve stands for the $60\%$ confidence band given by the PLDS model. The time series is the average of signals from 20 randomly picked voxels.}
\label{fig:samplets}
\end{center}
...
%\end{center}
%\subsection{Eigen Values Of Data Matrix}
%The eigen values plot is given in the following figure
%\begin{center}
...
\hline
0. Define $\mathbf{x}_t^{\tau}$ = E($\mathbf{x}_t|\{\mathbf{y}\}_1^{\tau}$),$\mathbf{V}_t^{\tau}=\text{Var}(\mathbf{x}_t|\{\mathbf{y}\}_1^{\tau})$, $\hat{\mathbf{x}}_t \equiv \mathbf{x}_t^T$ and $P_t\equiv V_t^T+\mathbf{x}_t^T{\mathbf{x}_t^T}^{\prime}$\\
...
\hspace{4 mm} $K_t=\mathbf{V}_t^{t-1}C^{\prime}(CV_t^{t-1}C^{\prime}+R)^{-1}$\\
...
\hspace{4 mm} $J_{t-1} = V_{t-1}^{t-1}A^{\prime}(V_t^{t-1})^{-1}$\\
\hspace{4 mm} $\mathbf{x}_{t-1}^T=\mathbf{x}_{t-1}^{t-1}+J_{t-1}(\mathbf{x_t^T-A\mathbf{x}_{t-1}^{t-1}})$\\
\hspace{4 mm} $V_{t-1}^T = V_{t-1}^{t-1}+J_{t-1}(V_t^T-V_t^{t-1})J_{t-1}^{\prime}$\\
\hspace{4 mm} $P_{t,t-1}\equiv V_{t,t-1}^T+\mathbf{x}_t^T{\mathbf{x}_t^T}^{\prime}$\\
